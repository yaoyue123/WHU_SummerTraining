...
direction_t bfs(game_t *game, int **life, int startx, int starty, int endx, int endy, int *dist, direction_t *initDir) {
...
direction_t get_action(game_t* game)
{
    ...
    for(int i = 0; i < game->board->rows; i++) {
        for(int j = 0; j < game->board->cols; j++) {
            if(game->board->cells[i * game->board->cols + j] == CELL_FOOD) {
                ...
                life = InitLife(game);
                direction_t dir = bfs(game, life, game->snake->head->row, game->snake->head->col, i, j, &dist, &initDir);
                if(dist == -1) {
                ...
                int dist2;
                bfs(game, life, i, j, tail_x, tail_y, &dist2, &initDir);
                if(dist2 == -1) {
                    DestroyLife(game, life);
                    continue;
                }
    ...
    if(mindist == 999999999) {
        ...
        int **life;
        life = InitLife(game);
        best_dir = bfs(game, life, game->snake->head->row, game->snake->head->col, game->snake->tail->row, game->snake->tail->col, &dist, &initDir);
        DestroyLife(game, life);

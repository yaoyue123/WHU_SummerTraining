...
class AgentRules:
  ...
  def applyAction( state, action, agentIndex ):
    ...
    agentState.isPacman = (agentIndex < 2)
      ...
      if agentState.numCarrying > 0:# and not agentState.isPacman:
        ...
        for index in range(state.getNumAgents()):
          ...
          if index in state.getRedTeamIndices():
            ...
            blueCount += agentState.numReturned
        if redCount + blueCount == TOTAL_FOOD:
            state.data._win = True
  ...
  def checkDeath( state, agentIndex):
    ...
    if agentState.isPacman:
      for index in otherTeam:#[0, 1, 2, 3]:
...
def readCommand( argv ):
  ...
  if options.textgraphics:
    ...
    args['display'] = captureGraphicsDisplay.PacmanGraphics(options.red, options.blue, options.zoom, 0.1, capture=True)
    import __main__
